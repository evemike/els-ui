...
const doEval = (s = "", obj = {}, isGetRefValue = false) => {
  try {
    ...
    const vals = keys.map((k) => isGetRefValue ? vue.unref(obj[k]) : obj[k]);
    const str = "return " + s;
...
const parseDirective = (directive, scope) => {
  ...
  keys.forEach((k) => {
    ...
    if (REG_BIND.test(k)) {
      ...
      if (ek[1]) {
        const tk = "on" + lodash.upperFirst(ek[1]);
...
const specialRender = {
  ...
  slot({ props, children, slots }) {
    ...
    return vue.renderSlot(slots, name, props, () => childs());
  },
  component({ props, children }) {
    const name = props.is;
    delete props.is;
    const c = vue.resolveDynamicComponent(name);
    return vue.createVNode(c, props, children);
  }
};
const defaultRender = (config) => {
  ...
  if (directives["v-text"]) {
    return vue.h(tag, props, directives["v-text"]);
  }
  return vue.h(tag, props, children);
};
const propsFilter = (props, extKeys) => {
  return lodash.omit(props, extKeys);
};
exports.defaultRender = defaultRender;
exports.getDestruct = getDestruct;
exports.getElemAttrs = getElemAttrs;
exports.getExpValue = getExpValue;
exports.getSlotName = getSlotName;
exports.isHtmlTag = isHtmlTag;
exports.parseDirective = parseDirective;
exports.propsFilter = propsFilter;
exports.specialRender = specialRender;
